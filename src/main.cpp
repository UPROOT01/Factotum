...
class E09 : public BaseProject {
  ...
  DescriptorSet DSDrill;
    ...
    DSDrill.init(this, &DSLlocalPBR, {SC.T[0]->getViewAndSampler(), SC.T[1]->getViewAndSampler(), SC.T[3]->getViewAndSampler(), SC.T[2]->getViewAndSampler()});
...
//printMat4("TF[55]", (*TMsp)[55]);
    ...
    ubos.mMat   = glm::translate(glm::mat4(1.f), glm::vec3(1.f, 3.f, 0.f)) * Mdrill.Wm;
    ubos.mvpMat = ViewPrj * ubos.mMat;
    ubos.nMat   = glm::inverse(glm::transpose(ubos.mMat));
